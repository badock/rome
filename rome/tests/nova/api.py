...
def _context_manager_from_context(context):
    # from context import RomeContextManager
    from context import RomeTransactionContext
    return RomeTransactionContext()
...
def model_query(context, model,
    ...
    from context import RomeContextManager
    from context import RomeRequestContext
    from rome.core.orm.query import Query as RomeQuery
    ...
    if isinstance(args, tuple):
        ...
        args = [model, context.session] + (args if args is not None else [])
        query = RomeQuery(*args)
