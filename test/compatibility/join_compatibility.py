...
def testing_join_1(models_module):
    query = models_module.get_query(models_module.Author).join(models_module.Book)
    if models_module is sqlalchemy_models:
...
def testing_join_2(models_module):
    query = models_module.get_query(models_module.Author, models_module.Book).join(models_module.Book)
    if models_module is sqlalchemy_models:
        print get_literal_query(query)
    extract_column = lambda x: getattr(x, "__tablename__", str(x))
    extract_row = lambda x: map(lambda y: extract_column(y), x) if hasattr(x, "__iter__") else extract_column(x)
...
def testing_join_3(models_module):
    query = models_module.get_query(models_module.Author).join(models_module.Author.books)
    if models_module is sqlalchemy_models:
        print get_literal_query(query)
    extract_column = lambda x: getattr(x, "__tablename__", str(x))
    extract_row = lambda x: map(lambda y: extract_column(y), x) if hasattr(x, "__iter__") else extract_column(x)
...
def testing_join_4(models_module):
...
def testing_join_5(models_module):
    query = models_module.get_query(models_module.Author).join(models_module.Author.books).options(
...
def testing_join_6(models_module):
    query = models_module.get_query(models_module.Author, models_module.Author.name).join(models_module.Author.books).options(
        subqueryload(models_module.Author.books)).filter(models_module.Author.name == 'Author3').filter(
        models_module.Book.title == 'Book3_5')
    if models_module is sqlalchemy_models:
        # query.column_descriptions[0]["entity"](**{"id":1, "name": "toto"})
